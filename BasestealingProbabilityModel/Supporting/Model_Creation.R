...
x_data <- StolenBaseData_Shortened %>% select(
  ...
  pitch_classification = factor(pitch_classification),
  target_base = factor(target_base)
)
cols <- c("Lead_Distance_Gained", "Pitcher_First_Move", "At_Pitch_Release")
x_data[cols] <- lapply(x_data[cols], as.numeric) 


# Create X and Y values for Model Training
...
# Create Final Model
dfull <- xgb.DMatrix(data = x_matrix,label = y)
params <- list(
...
BaseStealing_Model_XGB <- xgb.train(
  params = params,
  data = dfull,
  nrounds = 300
)

# Save Model and dumm variables
saveRDS(BaseStealing_Model_XGB, "stolen_base_xgb_model.rds")
saveRDS(dummy, "stolen_base_dummy_vars.rds")
